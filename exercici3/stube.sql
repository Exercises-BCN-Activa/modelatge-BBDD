-- MySQL Script generated by MySQL Workbench
-- Mon May  4 18:38:18 2020
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';

-- -----------------------------------------------------
-- Schema stube
-- -----------------------------------------------------
DROP SCHEMA IF EXISTS `stube` ;

-- -----------------------------------------------------
-- Schema stube
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `stube` DEFAULT CHARACTER SET utf8 ;
USE `stube` ;

-- -----------------------------------------------------
-- Table `stube`.`usuaris`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `stube`.`usuaris` ;

CREATE TABLE IF NOT EXISTS `stube`.`usuaris` (
  `idusuaris` VARCHAR(15) NOT NULL,
  `email` VARCHAR(60) NOT NULL,
  `contrasenya` VARCHAR(10) NOT NULL,
  PRIMARY KEY (`idusuaris`),
  UNIQUE INDEX `email_UNIQUE` (`email` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `stube`.`videos`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `stube`.`videos` ;

CREATE TABLE IF NOT EXISTS `stube`.`videos` (
  `idvideos` VARCHAR(12) NOT NULL,
  `titol` VARCHAR(100) NOT NULL,
  `descripcio` MEDIUMTEXT NOT NULL,
  `propietari` VARCHAR(15) NOT NULL,
  PRIMARY KEY (`idvideos`),
  CONSTRAINT `idusuaris`
    FOREIGN KEY (`propietari`)
    REFERENCES `stube`.`usuaris` (`idusuaris`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT)
ENGINE = InnoDB;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
